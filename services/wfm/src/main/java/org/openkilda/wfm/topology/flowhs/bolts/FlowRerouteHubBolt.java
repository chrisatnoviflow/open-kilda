...
public class FlowRerouteHubBolt extends HubBolt {
    ...
    protected void init() {
        ...
        service = new FlowRerouteService(persistenceManager, pathComputer, resourcesManager);
    }

    @Override
    protected void onRequest(Tuple input) throws PipelineException {
        String key = input.getStringByField(MessageTranslator.FIELD_ID_KEY);
        String flowId = (String) input.getValueByField(FIELD_ID_PAYLOAD);
        service.handleRequest(key, pullContext(input), flowId, new FlowRerouteHubCarrierImpl(input));
    ...
    private class FlowRerouteHubCarrierImpl implements FlowRerouteHubCarrier {
        private Tuple tuple;

        public FlowRerouteHubCarrierImpl(Tuple tuple) {
            this.tuple = tuple;
        }
        ...
        public void sendHistoryUpdate(FlowHistoryHolder historyHolder) {
            //TODO: emitWithContext(Stream.HUB_TO_HISTORY_BOLT.name(), tuple, new Values(null, historyHolder));
        }
    }
}
