...
public class DeallocateResourcesAction extends
    ...
    public void execute(FlowRerouteFsm.State from, FlowRerouteFsm.State to,
        ...
        try {
            FlowResources oldResources = stateMachine.getOldResources();
            resourcesManager.deallocatePathResources(oldResources);

            log.debug("Resources has been de-allocated: {}", oldResources);

            saveHistory(oldResources, stateMachine);
        } catch (Exception e) {
