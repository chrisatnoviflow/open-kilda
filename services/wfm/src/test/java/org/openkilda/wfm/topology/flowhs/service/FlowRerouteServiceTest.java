...
public class FlowRerouteServiceTest {
    ...
    public void setUp() {
        ...
        when(flowRepository.exists(any())).thenReturn(true);
    ...
    public void shouldFailRerouteFlowIfNoPathAvailable()
        ...
        when(pathComputer.getPath(any())).thenThrow(new UnroutableFlowException("No path found"));
        buildFlowResources();

        FlowRerouteService rerouteService = new FlowRerouteService(persistenceManager,
                pathComputer, flowResourcesManager);

        rerouteService.handleRequest("test_key", commandContext, FLOW_ID, carrier);
        ...
        verify(pathComputer, times(1)).getPath(any());
    ...
    public void shouldFailRerouteFlowIfNoResourcesAvailable()
            throws RecoverableException, UnroutableFlowException, ResourceAllocationException {
        Flow flow = build2SwitchFlow();
        when(pathComputer.getPath(any())).thenReturn(build2SwitchPathPair(2, 3));
    ...
    public void shouldSkipRerouteIfNoNewPathFound()
            throws RecoverableException, UnroutableFlowException, ResourceAllocationException {
        Flow flow = build2SwitchFlow();
        when(pathComputer.getPath(any())).thenReturn(build2SwitchPathPair());
        buildFlowResources();
    ...
    private FlowResponse buildResponseOnGetInstalled(GetInstalledRule request) {
        ...
        if (rule instanceof InstallEgressRule) {
            ...
            builder.inVlan(((InstallIngressRule) rule).getInputVlanId());
        }
